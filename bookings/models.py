...
class Booking(models.Model):
    ...
    def clean(self):
        ...
        existing_bookings = Booking.objects.filter(
            meal_time=self.meal_time, meal_day=self.meal_day)
        if self.pk:
            existing_bookings = existing_bookings.exclude(pk=self.pk)
        if existing_bookings.exists():
            raise ValidationError(
                "A booking already exists at this time on this day.")
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            ...
            booked_times = Booking.objects.filter(
                meal_day=self.meal_day).values_list('meal_time', flat=True)
            available_times = [
                time for time in available_times if time not in booked_times]
